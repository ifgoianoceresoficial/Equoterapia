...
function PraticantesArquivados() {
  ...
  const handleReactivate = async (id) => {
    if (!window.confirm("Tem certeza que deseja reativar este praticante?")) {
      return;
    }
    try {
      await api.put(`/pacients/${id}/changeStatus`);
      setPraticantes((prev) => prev.filter((p) => p.id !== id));
    } catch (error) {
      console.error("Erro ao reativar praticante:", error);
    }
  };
      ...
      <div className="content">
        ...
        <div className="conteudo-lista list-group">
          {filteredPraticantes.map((praticante, index) => {
            const dataNascimento = praticante.birthDate ? new Date(praticante.birthDate + "T00:00:00") : null;
            ...
            return (
              ...
              >
                <div className="d-flex align-items-center gap-5">
                  <Link to={`/InformacoesPraticante/${praticante.id}`}>
                    <img
                  ...
                  <div className="me-5">
                    <h5 className="mb-0">
                      <Link to={`/InformacoesPraticante/${praticante.id}`} style={{ textDecoration: 'none', color: 'inherit', fontWeight: 'bold' }}>
                        {praticante.name}
                      </Link>
                    </h5>
                  </div>
                ...
                <div className="d-flex align-items-center">
                  ...
                  <button 
                    ...
                    onClick={() => handleReactivate(praticante.id)}
                  >
                    Reativar praticante
                  </button>
                </div>
